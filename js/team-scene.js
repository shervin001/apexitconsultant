...
import { loadImage, computePersonSample } from './portrait-pipeline.js';
...
if (v2Link) v2Link.href = `themes_v2.html?p=${encodeURIComponent(personKey)}`;
...
async function buildCloud(view, viewIndex, viewTotal) {
  ...
  const { minX, minY, maxX, maxY, bw, bh } = bbox;
  const cxN = bbox.cx;
  const cyN = bbox.cy;
